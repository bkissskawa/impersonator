 ...
 *
 * Copied from https://github.com/jsslkeylog/jsslkeylog
 */
public class SslKeyLogWriter {
	...
	public static void logRSA(byte[] encryptedPreMasterSecret, TlsSecret preMasterSecret) {
		logLine("RSA " + hex(encryptedPreMasterSecret).substring(0, 16) + " " + hex(((AbstractTlsSecret) preMasterSecret).copyData()), null);
	}

	public static void logClientRandom(byte[] clientRandom, TlsSecret masterSecret, Object maybeConn) {
		logClientRandom(clientRandom, masterSecret, maybeConn instanceof SSLSocket ? (SSLSocket) maybeConn : null);
	}
	
	public static void logClientRandom(byte[] clientRandom, TlsSecret masterSecret, SSLSocket conn) {
		logLine("CLIENT_RANDOM " + hex(clientRandom) + " " + hex(((AbstractTlsSecret) masterSecret).copyData()),
				conn == null ? null : conn.getLocalSocketAddress() + " -> " + conn.getRemoteSocketAddress());
	}

	public static void logBcClientRandom(byte[] clientRandom, byte[] masterSecret, SSLSocket conn) {
